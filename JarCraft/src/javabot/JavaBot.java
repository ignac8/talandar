...
public class JavaBot implements BWAPIEventListener {
    ...
    public Point getBuildTile(int builderID, int buildingTypeID, int x, int y) {
        ...
        if (ret.x == -1)
            bwapi.printText("Unable to find suitable build position for " + bwapi.getUnitType(buildingTypeID).getName());
        return ret;
    }
    ...
    public boolean weAreBuilding(int buildingTypeID) {
        for (Unit unit : bwapi.getMyUnits()) {
            ...
            if (bwapi.getUnitType(unit.getTypeID()).isWorker() && unit.getConstructingTypeID() == buildingTypeID)
                return true;
        }
        return false;
    }
